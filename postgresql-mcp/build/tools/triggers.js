 ...
 */
export async function getTriggers(connectionString, tableName, schema = 'public') {
    ...
    `;
        ...
        const triggers = await db.query(query, params);
        return {
            success: true,
            message: tableName
 ...
 * Create a trigger
 */
export async function createTrigger(connectionString, triggerName, tableName, functionName, options = {}) {
    const db = DatabaseConnection.getInstance();
    try {
        await db.connect(connectionString);
 ...
 * Drop a trigger
 */
export async function dropTrigger(connectionString, triggerName, tableName, options = {}) {
    const db = DatabaseConnection.getInstance();
    try {
        await db.connect(connectionString);
        const schema = options.schema || 'public';
 ...
 */
export async function setTriggerState(connectionString, triggerName, tableName, enable, options = {}) {
    const db = DatabaseConnection.getInstance();
    try {
        await db.connect(connectionString);
        const schema = options.schema || 'public';
